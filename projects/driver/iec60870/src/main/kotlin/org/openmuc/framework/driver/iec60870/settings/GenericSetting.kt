...
abstract class GenericSetting {
            ...
            try {
                for (option in options.enumConstants) {
                    ...
                    for (singlesetting in settingsArray) {
                        ...
                        if (pairLength == 2 && pair[0].trim { it <= ' ' } == prefix) {
                            try {
    ...
    """.trimIndent()
                    ...
                    if (noOptionsPresent && mandatory) {
                        throw ArgumentSyntaxException(
                            "Mandatory parameter " + option.name + " is nor present in "
...
"""
    ...
    @Synchronized
    @Throws(ArgumentSyntaxException::class)
    private fun extractByteArray(value: String, errorMessage: String): ByteArray {
        var ret = byteArrayOf()
        if (value.startsWith("0x")) {
            ...
            ret = value.toByteArray(StandardCharsets.US_ASCII)
        }
        return ret
    }
    ...
    companion object {
        ...
        private fun fromShortHexString(shortHexString: String): ByteArray {
            validate(shortHexString)
            val length = shortHexString.length
            val data = ByteArray(length / 2)
            var i = 0
            while (i < length) {
                val firstCharacter = shortHexString[i].digitToIntOrNull(16) ?: -1
                val secondCharacter = shortHexString[i + 1].digitToIntOrNull(16) ?: -1
                if (firstCharacter == -1 || secondCharacter == -1) {
                    throw NumberFormatException("string is not a legal hex string.")
                }
                data[i / 2] = ((firstCharacter shl 4) + secondCharacter).toByte()
                i += 2
            }
            return data
        }
        ...
        private fun validate(s: String?) {
            requireNotNull(s) { "string s may not be null" }
